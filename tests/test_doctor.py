...
class CheckConfigSyntaxTest(unittest.TestCase):
    ...
    def test_installed_unregistered_agent_produces_fixable_warning(self) -> None:
        ...
        with patch(
            "aikito.doctor.detect_existing_agents",
...
""".strip()
    ...
    def test_unreadable_mcp_config_reports_fail_instead_of_crashing(self) -> None:
        ...
        with patch.object(Path, "read_text", side_effect=mock_read_text, autospec=True):
            from aikito.doctor import check_config_syntax, check_drift
    ...
    def test_check_security_windows_developer_mode(self) -> None:
        (self.aikito_dir / ".gitignore").write_text("/.local/\n")
        with patch("aikito.doctor.is_windows", return_value=True):
            with patch("aikito.doctor.can_symlink", return_value=True):
            ...
            with patch("aikito.doctor.can_symlink", return_value=False):
                sec = check_security(self.aikito_dir, self.home)
                self.assertTrue(
                    any(
