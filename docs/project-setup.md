...
- **Offline paths & Dormant projects**: Candidate directories that do not exist locally (for example,
  ...
  When NO candidate paths exist on the current host, the project is considered **dormant** on this host.
  `aikito status` displays dormant status with a dimmed badge (`–`), and `aikito doctor` reports the project as healthy
  (`Project '<name>': dormant on this host (<candidates>)`), preserving candidate paths for seamless roaming across multiple machines through Git without false-positive failures.
