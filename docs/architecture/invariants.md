...
### INV-MEM-01: Memory Link-Only Contract `[planned]` {: #inv-mem-01 }
...
### INV-MEM-02: Workspace Memory Reference Identity `[planned]` {: #inv-mem-02 }
...
### INV-MEM-03: Project Notes Canonical Source Priority `[planned]` {: #inv-mem-03 }
...
### INV-MEM-04: Project Notes Directory-Only Scoping `[planned]` {: #inv-mem-04 }
...
### INV-MEM-05: Exact Canonical Symlink for Memory Ownership `[planned]` {: #inv-mem-05 }
...
### INV-MEM-06: Deselected Memory Stale Entry Cleanup `[planned]` {: #inv-mem-06 }
...
### INV-MEM-07: Memory Conflict and Preservation Guarantee `[planned]` {: #inv-mem-07 }
...
- **Target Model (`[planned]`)**: In the unified link planning engine, any pre-existing unmanaged file, directory, or external symlink at `.agents/memory/<name>` evaluates to `CONFLICT`, strictly preserving the target and emitting a diagnostic finding.
- **Legacy Compatibility Gap (`[compat-gap]`)**: In the `1.46.0` baseline, legacy `sync_resource()` unconditionally unlinked or removed existing files, directories, or symlinks at the target path before creating the symlink (`tests/test_memory_characterization.py::test_baseline_selected_memory_target_regular_file_overwritten_by_legacy`). Phase 6 eliminates this unsafe automatic replacement.

### INV-MEM-08: Multi-Checkout and Offline Memory Scope `[planned]` {: #inv-mem-08 }
...
### INV-MEM-09: Stale Plan Invalidation on Source or Target Mutation `[planned]` {: #inv-mem-09 }
...
### INV-MEM-10: Idempotent Convergence and Dry-Run Zero Mutation for Memory `[planned]` {: #inv-mem-10 }
...
### INV-MEM-11: Memory Result Segmentation `[planned]` {: #inv-mem-11 }
...
### INV-MEM-12: `Project.prepare` Memory Contract `[planned]` {: #inv-mem-12 }
