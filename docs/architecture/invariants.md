...
- **Target Model (`[current]`)**: In the unified link planning engine, any pre-existing unmanaged file, directory, or external symlink at `.agents/memory/<name>` evaluates to `CONFLICT`, strictly preserving the target and emitting a diagnostic finding.
