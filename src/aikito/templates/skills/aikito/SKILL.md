...
The public Python `Project.prepare()` API is the execution-facing entry for an
external runner. It requires exactly one active path (or an explicit `path`
override), applies the same persistent project sync rules, and returns the
resolved working directory. V1 supports Pi and never synchronizes global
resources, MCP servers, or subagents as part of preparation. An explicit path
may be an existing unregistered directory for an ephemeral CI or deployment
checkout; the caller is responsible for selecting the correct path.
